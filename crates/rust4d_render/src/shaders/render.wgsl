...
// ============================================================================
// Alternative Fragment Shader: Wireframe Mode
// ============================================================================

// This can be used for debugging or stylistic rendering
@fragment
fn fs_wireframe(input: VertexOutput) -> @location(0) vec4<f32> {
    // Use W-depth color directly without lighting
    let w_color = w_depth_to_color(input.w_depth, uniforms.w_range);
    return vec4<f32>(w_color, 1.0);
}

// ============================================================================
// Alternative Fragment Shader: Normal Visualization
// ============================================================================
...
// ============================================================================
// Alternative Fragment Shader: Pure W-Depth
// ============================================================================

// Shows only the W-depth coloring, ignoring vertex colors
@fragment
fn fs_w_depth_only(input: VertexOutput) -> @location(0) vec4<f32> {
    let normal = normalize(input.world_normal);
    let light_dir = normalize(uniforms.light_direction);
    let n_dot_l = max(dot(normal, light_dir), 0.0);
    let light = uniforms.ambient_strength + n_dot_l * uniforms.diffuse_strength;

    let w_color = w_depth_to_color(input.w_depth, uniforms.w_range);
    return vec4<f32>(w_color * light, 1.0);
}
